...
async def _search_jobs_with_backoff(
    ...
    try:
        ...
        logger.info(f"[Adzuna] Query → what: '{what_query}' (no what_or for individual variant search)")
...
async def get_job_role_descriptions_function(
    ...
    country_code = country_code.lower()  # ✅ Normalize to lowercase
    ...
    if jobs:
        ...
        _ingest_jobs_data(jobs, country_code)  # ✅ Normalized country_code passed
        return {
...
async def summarise_expanded_job_roles_tool(
    ...
    async def fetch_title_data(title: str):
        ...
        return await get_job_role_descriptions_function(
            job_title=title,
            country_code=country_code,  # ✅ Lowercased already
            location=location,
            salary_min=salary_min,
            employment_type=employment_type,
