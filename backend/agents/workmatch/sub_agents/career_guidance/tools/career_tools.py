...
async def _search_jobs_with_backoff(
    job_title: str, country_code: str, location: Optional[str],
    salary_min: Optional[int], employment_type: Optional[str]
) -> List[Dict[str, Any]]:
    what_query = " ".join(job_title.split())
    jobs = adzuna_api.search_jobs(
        what=what_query, what_or=what_query, country=country_code,
        location=location, salary_min=salary_min, employment_type=employment_type
    ).get("results", [])
...
async def get_job_role_descriptions_function(
    ...
    location: Optional[str] = None, salary_min: Optional[int] = None, employment_type: Optional[str] = None
) -> dict:
    ...
    jobs = await _search_jobs_with_backoff(job_title, country_code, location, current_salary_min, employment_type)
    if not jobs:
        return {"message": f"I couldn't find any current openings for '{job_title}' or similar roles."}
    _ingest_jobs_data(jobs, country_code)
    return {"job_description_examples": []}  # Placeholder
...
async def explore_career_fields_function(
    ...
    if "high paying" in keywords.lower():
        threshold = HIGH_PAYING_THRESHOLDS.get(country_code, DEFAULT_HIGH_PAYING_THRESHOLD)
        current_salary_min = max(salary_min or 0, threshold)
    jobs = await _search_jobs_with_backoff(keywords, country_code, location, current_salary_min, employment_type)
    if not jobs:
...
async def ingest_jobs_from_adzuna(
    ...
    if "high paying" in what.lower():
        threshold = HIGH_PAYING_THRESHOLDS.get(country_code, DEFAULT_HIGH_PAYING_THRESHOLD)
        current_salary_min = max(salary_min or 0, threshold)
    jobs = await _search_jobs_with_backoff(what, country_code, location, current_salary_min, employment_type)
    if not jobs:
